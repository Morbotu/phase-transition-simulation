...
def dT_liquid(T):
    ...
    with objmode(y="float64[:]"):
        y = solve_banded((1, 1), diagonals, T)
...
def solve_equation(x, T0, dT):
    ...
    while True:
        ...
        if np.any(dT_solid(T[left-1:right+2]) < 0):
            print(dT_solid(T[left-1:right+2])[dT_solid(T[left-1:right+2]) < 0])
...
T0 = [100.] # *C # left boundary condition temperatures
dT = [.5] # *C # mushy zone region. Higher boundary condition requires higher dT. Check theta_l graph for realistic distribution
T = [] # results array
T_analytics = []
